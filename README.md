...
## Что уже реализовано для демонстрации
...
1. Запустить бэкенд (см. раздел «Как запустить бэкенд»).
2. Запустить фронтенд (см. раздел «Как запустить фронтенд»).
3. В браузере:
   - выбрать доступный сценарий («Аварийное отключение реактора»),
   - пошагово отвечать на вопросы,
   - показать, как система приводит к различным финальным действиям
     в зависимости от ответов оператора.

Это минимально рабочий прототип, демонстрирующий:
- хранение сценария в БД,
- детерминированное дерево решений,
- диалоговый интерфейс для оператора.

## Связка с GitHub

1. На [github.com/new](https://github.com/new) создайте **пустой** репозиторий (без README и .gitignore, если уже инициализировали git локально).
